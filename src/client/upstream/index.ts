...
export function apply(ctx: ClientContext, options: { multiroot?: boolean } = {}): void {
  // The stock dsh-client-ui-workspace owns the "workspace" namespace on dsh
  // 0.1.2.x; coexist instead of failing the whole entry.
  ctx.effect(() => {
    try {
      ctx.locale.register(NS, { zh, en })
    } catch (error) {
      if (!String((error as Error)?.message).includes('already has locale')) throw error
    }
